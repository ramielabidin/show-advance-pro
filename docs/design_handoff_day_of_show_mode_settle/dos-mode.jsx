...
function DayOfShowMode({state, setState, onClose, hotelVariant='editorial', settleStyle='slide'}){
  ...
  return (
    <div style={{
      ...
      <div key={phase} className="phase-morph" style={{
        ...
        {phase === 2 && <PhaseSettle state={state} setState={setState} settleStyle={settleStyle}/>}
...
function PhaseSettle({state, setState, settleStyle='slide'}){
  const data = window.DOS_DATA;
  ...
  return (
    <div style={{
      ...
      {settleStyle === 'spine' ? (
        <SpineSettle onConfirm={() => setState(s => ({...s, settled: true}))}/>
      ) : (
        <SlideSettle onConfirm={() => setState(s => ({...s, settled: true}))}/>
      )}
      ...
      {/* Footer: the night as a thin timeline */}
      <div style={{marginTop:'auto', paddingTop:24}}>
        <NightTimeline schedule={data.schedule}/>
      </div>
    </div>
  );
}

/* ----- Slide to settle (variant E) ----- */
function SlideSettle({onConfirm}){
  return (
    <div style={{display:'flex', flexDirection:'column', gap:14, padding:'4px 0'}}>
      ...
      <SlideToSettle onConfirm={onConfirm}/>
    </div>
  );
}

/* ----- Spine settle (variant D) — quiet card with green spine ----- */
function SpineSettle({onConfirm}){
  return (
    <div
      className="pressable"
      onClick={onConfirm}
    ...
    >
      ...
      <div style={{display:'flex', flexDirection:'column', gap:14}}>
        ...
        <div style={{
          ...
          Open settle <M_Icon name="arrow-right" size={13} stroke={2}/>
        </div>
      </div>
    </div>
  );
...
/* ----- Slide-to-settle interactive primitive ----- */
function SlideToSettle({onConfirm}){
  ...
  const updateFromClient = React.useCallback((clientX) => {
    const el = trackRef.current; if(!el) return;
    const r = el.getBoundingClientRect();
    const usable = r.width - KNOB - PAD*2;
    const x = Math.max(0, Math.min(usable, clientX - r.left - PAD - KNOB/2));
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background:`linear-gradient(90deg, hsl(var(--success) / ${0.18 + progress*0.42}) 0%, hsl(var(--success) / 0) ${Math.max(20, 60 - progress*30)}%)`,
      ...
      <div style={{
        ...
        {confirmed ? 'Opening…' : 'Slide to settle'}
        <M_Icon name="arrow-right" size={15} stroke={1.6} style={{marginLeft:10, opacity:0.55}}/>
      </div>
      {/* Knob */}
      <div
        onPointerDown={start}
        onTouchStart={start}
        className={progress > 0.05 ? '' : 'mic-glow'}
        style={{
          position:'absolute',
          top:PAD, left:PAD,
      ...
      >
        <M_Icon name={confirmed ? 'check' : 'arrow-right'} size={20} stroke={2.2}/>
      </div>
    </div>
  );
}

/* ----- Night timeline (DNA footer) ----- */
function NightTimeline({schedule}){
  const startMin = hmToMinutes(schedule[0].time);
  const norm = (t) => { let m = hmToMinutes(t); if(m < startMin) m += 24*60; return m; };
  const endMin = norm(schedule[schedule.length-1].time);
  ...
  return (
    <div>
      <div style={{
        ...
        <span style={{fontFamily:'var(--font-mono)', textTransform:'none', letterSpacing:0, fontSize:11}}>{schedule.length} cues</span>
      </div>
      <div style={{position:'relative', height:32}}>
        ...
        {schedule.map((s) => {
          ...
          return (
            <div key={s.id} title={`${fmt12short(s.time)} — ${s.label}`} style={{
      ...
      <div style={{
        ...
        <span>{fmt12short(schedule[0].time)}</span>
        <span>{fmt12short(schedule[schedule.length-1].time)}</span>
      </div>
    </div>
  );
}
