...
| `dos-mode.jsx` | The Day of Show surface — three phase components, plus `SlideToSettle` (variant E) and `SpineSettle` (variant D) primitives |
| `dos-primitives.jsx` | `Icon`, `Eyebrow`, `Chip`, `MicChip`, `StatusBar` |
| `dos-app.jsx` | Root: phone frame stage + scrubber + Tweaks wiring (includes the `settleStyle: 'slide' \| 'spine'` toggle) |
| `canvas-frames.jsx` | Variants canvas composition |
| `phase2-variants.jsx` | Phase-2 footer treatments explored in review (current is timeline DNA, variant C) |
| `phase2-settle-variants.jsx` | Settle CTA treatments explored in review (current is slide, variant E; spine is variant D) |
...
3. **Settle CTA** — ship one of two treatments, see below. Default is **E (slide-to-settle)**; **D (spine)** is the recommended fallback if a swipe gesture conflicts with platform conventions.
4. **Hotel teaser** (margin-top 22) — unchanged from earlier draft
5. **Night-timeline footer** (margin-top auto, pushed to bottom)

### Settle CTA — variant E (slide-to-settle, default)

A pill-shaped track with a glowing knob. The user drags the knob right to commit; release past ~92% triggers settle, otherwise it snaps back. This is the canonical treatment — it pairs commitment ("you can't settle by accident") with the physical close-the-night metaphor.

```
Type-led headline (no card, no fill):
  Eyebrow: "Last thing"      500 10.5px DM Sans, 0.22em tracking, --muted-foreground
  Title:   "Settle this show."  DM Serif Display 44px/1, -0.03em
  Sub:     "Count the door, save your numbers. About five minutes."
           13.5px, --muted-foreground, line-height 1.45
  ↓ gap 14, then the slider:

Slider track:
  Height 60, border-radius 999
  Background: hsl(var(--card)), 1px --border
  Inset fill (animated by drag progress):
    linear-gradient(90deg,
      hsl(var(--success) / [0.18 + p*0.42]) 0%,
      hsl(var(--success) / 0) [60-p*30]%)

Track label (centered, padding-left 56 to clear knob):
  "SLIDE TO SETTLE" 11px DM Sans 500, 0.22em tracking, --muted-foreground
  + Lucide arrow-right 15px, opacity 0.55
  Opacity fades to 0 as drag progress crosses 0.6
  Swap to "OPENING…" once committed

Knob:
  52×52 circle, top 4 left 4 (4px padding inside track)
  Background: hsl(var(--success))
  Box-shadow: 0 6px 18px hsl(152 60% 30% / 0.5)
  Pulse glow when at rest (.mic-glow keyframes from prototype) — drops while dragging
  Icon: Lucide arrow-right 20px, white, stroke 2.2
  Swap to Lucide check 20px once committed

Drag math:
  usable = trackWidth - knobSize - 2*4 (padding)
  progress = clamp((clientX - trackLeft - 4 - knobSize/2) / usable, 0, 1)
  knob.transform = translateX(progress * 100%)
  On release: progress >= 0.92 → confirm (220ms delay then onConfirm), else snap to 0
  Snap transition: 260ms cubic-bezier(.2,.8,.2,1)
  While dragging: no transition (direct follow)

Pointer events: onPointerDown/onTouchStart on knob; window-level pointermove/pointerup,
touchmove/touchend listeners while dragging. touch-action: none on track.
```

**Accessibility:** keyboard fallback — pressing Enter/Space when the knob is focused should open settle directly (treat it like a button). Voice-over should announce as "Slide to settle, button" with a hint like "Double-tap to open settle." The drag is a power-user gesture; the button semantics are the source of truth.

### Settle CTA — variant D (spine, fallback)

Quiet card with a thin glowing green spine on the left edge. Whole card is the click target.
...
Spine (absolute left:0 top:0 bottom:0, width 4):
  Background: hsl(var(--success))
  Box-shadow: 0 0 24px 0 hsl(var(--success) / 0.55)

Inner stack (gap 14):
  Eyebrow: "Ready"             500 11px DM Sans, 0.18em tracking, hsl(var(--success))
  Title:   "Close the night."  DM Serif Display 38px/1, -0.03em
  Sub:     "Count the door, save your numbers. Five minutes."
           13.5px, --muted-foreground, line-height 1.45
  Pill action (align-self: flex-start, margin-top 6):
    Transparent bg, 1px --border, radius 999, padding 9px 16px 9px 18px
    "Open settle →"  500 13px DM Sans + Lucide arrow-right 13
```

Both treatments call the same `onConfirm` (open the existing settle surface).
...
### Night-timeline footer (replaces the old "Tonight" recap list)

A thin horizontal timeline visualizing the whole night as dots on a single rule. Replaces the verbose `<check> 6:30 pm Doors` list — same data, gentler form.

```
Pushed to bottom with margin-top: auto + padding-top: 24

Header row (flex justify-between):
  Left:  "Tonight, in full"  500 10px DM Sans, 0.18em tracking, --muted-foreground
  Right: "8 cues"             11px JetBrains Mono, --muted-foreground (no tracking, no caps)

Timeline (position relative, height 32):
  Rule: absolute top:50%, left:0, right:0, height:1, hsl(var(--border))
  Each schedule entry → dot, absolute-positioned at left:`${pos}%`, transform:translate(-50%,-50%)
  Position math:
    startMin = first entry's time
    norm(time) = entryMin < startMin ? entryMin + 24*60 : entryMin   // handles past-midnight curfew
    pos = ((norm(time) - startMin) / (norm(last) - startMin)) * 100
  Dot size & color:
    Band entries (is_band):  10×10, hsl(var(--badge-new)), with halo
                              box-shadow: 0 0 0 3px var(--background), 0 0 0 4px hsl(var(--badge-new) / 0.5)
    Other entries:           6×6,  hsl(var(--muted-foreground)),
                              box-shadow: 0 0 0 3px var(--background)  (knockout against rule)

Anchors row (flex justify-between, margin-top 6):
  Left  : first entry's time, 10.5px JetBrains Mono, --muted-foreground (e.g. "3:00pm")
  Right : last entry's time   (e.g. "12:30am")
```

The timeline is read-only in v1 — no tap-to-jump on dots, no tooltip beyond the native `title` attr (`"<time> — <label>"`).
...
- **DM Serif Display** — Hero countdown (90/110px), hotel name (46px), Settle title (44px slide / 38px spine), sign-off (22px), all dates on cards
...
- Spine settle card: `border-radius: 14px`
- Slide-to-settle track: `border-radius: 999px`
