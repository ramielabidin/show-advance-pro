-- Hotel stay dates: support multi-day hotel reservations.
--
-- Existing `hotel_checkin` / `hotel_checkout` are time-only strings
-- ("3:00 PM", "11:00 AM"). This migration adds matching `*_date`
-- columns so a stay can span multiple days. Existing rows are
-- unaffected — both new columns default to NULL, and consumers
-- fall back to the time-only display when a date isn't set.

-- 1) Add the new date columns ------------------------------------------------
ALTER TABLE public.shows
  ADD COLUMN IF NOT EXISTS hotel_checkin_date  date,
  ADD COLUMN IF NOT EXISTS hotel_checkout_date date;

-- 2) Replace get_guest_show to surface the new fields ------------------------
