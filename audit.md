...
| `src/components/EmptyState.tsx` | 16 | `<h3 className="text-lg font-medium text-foreground mb-1">` — no `font-sans` override; inherits `font-display`, so every empty-state title renders in DM Serif Display | Type | med | Add `font-sans` to className |
...
| `src/components/TourRevenueSimulator.tsx` | 84 | `<h3 className="text-xs font-medium uppercase tracking-wider text-muted-foreground">` — no `font-sans`; section-label typography in DM Serif Display at 12px uppercase is wrong | Type | med | Add `font-sans` |
| `src/components/EmailAttachments.tsx` | 84 | `<h3 className="text-sm font-medium text-foreground ...">` — no `font-sans`; section heading inherits `font-display` | Type | med | Add `font-sans` |
| `src/components/BandDocuments.tsx` | 116 | `<h2 className="font-medium text-foreground mb-0.5">Band Documents</h2>` — no `font-sans`; inherits `font-display` | Type | med | Add `font-sans` |
...
| `src/pages/SettingsPage.tsx` | 502, 558, 624, 707, 847 | Multiple `<h2 className="font-medium text-foreground ...">` section labels (Slack Integration, Home Base City, Email Forwarding, Touring Party, Team Members) — no `font-sans`; all render in DM Serif Display due to global rule | Type | med | Add `font-sans` to each, or fix the root `index.css` rule |
| `src/components/ShowCard.tsx` | 55 | `<h3 className="font-medium text-foreground text-sm sm:text-base truncate">` — venue name heading inherits `font-display` (DM Serif Display at 14–16px) where DM Sans is expected for a card data label | Type | med | Add `font-sans` |
...
| `src/components/BulkUploadDialog.tsx` | 742 | `bg-amber-500/5` — raw Tailwind color on a changed-row highlight | Color | low | Use `bg-[color-mix(in_srgb,var(--pastel-yellow-bg)_30%,transparent)]` or `bg-pastel-yellow/30` once the Tailwind alias is wired correctly |
...
| `src/components/TimeInput.tsx` | 88, 104 | `<SelectTrigger className="w-16 h-9 ...">` — time-picker selects in the schedule editor are used on mobile | Layout | low | Change to `h-11 sm:h-9` (may need `w-16 sm:w-auto` adjustment) |
...
### High-priority fixes (do these first — they affect every screen)

1. **`src/index.css:158–160`** — Remove the global `h1–h4 { font-display }` rule. It silently breaks font rendering across 8+ components. Fix once here; downstream h-tag violations disappear.
2. **`tailwind.config.ts`** — Add `pastel-purple` Tailwind alias. One-liner.
3. **`src/components/ShowCard.tsx:27–32, 76`** — Status dot and Settled badge both use raw Tailwind greens/reds/ambers. ShowCard renders on every list view.
4. **`src/components/BandDocuments.tsx:175`** — `confirm()` is the only remaining browser-native destructive dialog in the codebase.
